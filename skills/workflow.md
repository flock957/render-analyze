...
description: 完整渲染 Jank 分析工作流 - 从 trace 分析到竖屏长图截图报告（v4 portrait-longshot）
type: workflow
version: 4.0
---
...
- **为每个 top frame 增补证据字段**：
  ...
  - `evidence_slices` — Top-8 命中关键词 slice（name/thread/dur_ms/ts）
...
- Pin 完整渲染管线 track（9 条），详见 Pin 策略
- 每个问题输出 **2 张竖屏长图**：
...
- **detail 截图叠加标注框**：Pillow 在 target_ts 对应 x 区域画红色半透明高亮 +
...
- 根因分析来自 `FRAMEWORK_KB` 硬编码（6 种 jank 类型），**不依赖 LLM**
